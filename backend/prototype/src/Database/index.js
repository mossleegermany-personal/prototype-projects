...
// Find a single user by field — pass { email } or { userId }
export async function dbFind(query) {
  const sheets = await getSheets();
  const rows = await fetchRows(sheets);
  const found = rows.slice(1).find((row) => {
    if (query.email) return row[COL.EMAIL] === query.email;
    if (query.userId) return String(row[COL.USER_ID]) === String(query.userId);
    return false;
  });
...
export async function dbCreate({ name, email, hashedPassword }) {
  const sheets = await getSheets();
  const rows = await fetchRows(sheets);
  const ids = rows.slice(1).map((r) => parseInt(r[0], 10)).filter((n) => !isNaN(n));
  const userId = ids.length > 0 ? Math.max(...ids) + 1 : 1;
  const now = new Date();
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: 'RAW',
    insertDataOption: 'INSERT_ROWS',
    requestBody: { values: [[userId, name, email, hashedPassword, date, time]] },
  });

  return { userId, name, email, date, time };
}

// Update fields of an existing user row
export async function dbUpdate(userId, updates) {
  const sheets = await getSheets();
  const rows = await fetchRows(sheets);
  const rowIndex = rows.findIndex(
    (r, i) => i > 0 && String(r[COL.USER_ID]) === String(userId)
  );
  if (rowIndex === -1) return null;

  const existing = rows[rowIndex];
  const updatedRow = [
    existing[COL.USER_ID],
    updates.name      ?? existing[COL.NAME],
    updates.email     ?? existing[COL.EMAIL],
    updates.hashedPassword ?? existing[COL.PASSWORD],
    existing[COL.DATE],
    existing[COL.TIME],
  ];
...
export async function dbRemove(userId) {
  const sheets = await getSheets();
  const rows = await fetchRows(sheets);
  const rowIndex = rows.findIndex(
    (r, i) => i > 0 && String(r[COL.USER_ID]) === String(userId)
